...
def rename_outputs(results_dir, config_label, run_label):
    """
    Rename semua file output Rasa di results_dir dengan prefix config+run.
    Contoh: intent_report.json → TAv1_sparse_cv_run1_intent_report.json
    """
...
def compute_stability(summaries):
    ...
    for intent in sorted(all_intents):
        vals = [s["intent_per_class"][intent] for s in summaries
    ...
    for entity in sorted(all_entities):
        vals = [s["entity_per_class"][entity] for s in summaries
...
def print_stability(label, stab, train_f1=None):
    ...
    if "intent_overall_mean" in stab:
        ...
        if train_f1 is not None:
            ...
            elif gap > 0.05: verdict = "SEDIKIT OVERFIT (wajar untuk dataset kecil)"
            else:            verdict = "GOOD FIT"
            print(f"           Training F1: {train_f1:.4f}  |  Gap: {gap:.4f}  →  {verdict}")

        print(f"\n  Intent per class (mean ± std):")
        for intent, (mean, std) in stab.get("intent_per_class", {}).items():
            flags = []
            if mean < 0.80: flags.append("LEMAH")
            if std  > 0.05: flags.append("TIDAK STABIL")
    ...
    if "entity_overall_mean" in stab:
        ...
        for entity, (mean, std) in stab.get("entity_per_class", {}).items():
            flags = []
            if mean < 0.70: flags.append("LEMAH")
            if std  > 0.05: flags.append("TIDAK STABIL")
            flag_str = "  <-- " + ", ".join(flags) if flags else ""
...
def save_comparison_csv(all_results, session_dir):
    """
    Simpan 2 file CSV:
    1. comparison_overview.csv  — ringkasan overall F1 semua config (plus durasi)
    2. comparison_detail.csv    — F1 per intent & entity semua config (mudah di-sort di Excel)
    """

    # --- 1. Overview CSV ---
    overview_path = os.path.join(session_dir, "comparison_overview.csv")
    overview_rows = []
    # Sekarang all_results berisi tuple 5 elemen: (label, stab, train_f1, cv_dur, train_dur)
    for config_label, stab, train_f1, cv_dur, train_dur in all_results:
        ...
        if gap != "":
            if gap > 0.15:   verdict = "OVERFIT"
            elif gap > 0.05: verdict = "SEDIKIT OVERFIT"
            else:            verdict = "GOOD FIT"
        else:
            verdict = ""
    ...
    # Tentukan urutan kolom agar rapi
    fieldnames = [
        "config",
        "intent_f1_mean", "intent_f1_std",
        "entity_f1_mean", "entity_f1_std",
        "intent_train_f1", "gap_train_vs_cv", "overfit_verdict",
        "cv_duration_seconds", "train_duration_seconds"
    ]
    ...
    # --- 2. Detail CSV (per class, semua config dalam satu tabel) ---
...
def run_config(config_label, config_path, session_dir):
    """Jalankan N_RUNS CV + overfit detection untuk satu config, kembalikan
    stab, train_f1, cv_duration, train_duration."""
...
def main():
    ...
    for config_label, config_path in CONFIGS:
        if not os.path.exists(config_path):
            ...
            continue
        stab, train_f1, cv_dur, train_dur = run_config(config_label, config_path, session_dir)
    ...
    for config_label, stab, train_f1, cv_dur, train_dur in all_results:
        ...
        print(f"  {config_label:<20} {i_f1:>10.4f} {i_std:>6.4f} "
